...
#           supabase/migrations/. It also prints which pending local
...
case "$MODE" in
  ...
  deploy)
    ...
    # Bulk-deploy edge functions only when any exist (Phase 0 has none yet).
    if find supabase/functions -mindepth 2 -maxdepth 2 -name index.ts 2>/dev/null | grep -q .; then
      echo "Deploying all edge functions..."
      supabase functions deploy
      echo "All edge functions deployed from this commit."
    else
      echo "No edge functions in repo yet; skipping function deploy."
    fi
    ;;
  *)
