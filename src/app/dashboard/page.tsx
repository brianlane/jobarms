...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: sub }, { data: usage }] = await Promise.all([
    ...
    supabase.from("arm_run_usage").select("runs_used").eq("user_id", user!.id).eq("month_key", monthKey()).maybeSingle()
  ]);
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <p className="text-sm text-slate-500">Arm runs this month</p>
