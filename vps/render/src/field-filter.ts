...
/**
 * The page-wide `body` sweep the self-healing recovery uses grabs EVERY input on
 * the page, including things that are not application questions: the site search
 * box, the resume file input (handled separately by attachResume), the hidden
 * captcha token textarea, and honeypots. Surfacing those as review questions
 * ("needs your answer") is noise. This drops them from the answerable set. It
 * does NOT affect the sanity check (which runs on raw fields) or the fill path
 * for the resume/captcha (both operate on the live DOM directly).
 *
 * MIGRATED from workers/apply-arm/src/field-filter.ts unchanged.
 */
...
export function isNonApplicationField(f: FormField): boolean {
  ...
  // Resume/file upload: the arm attaches the resume against the live DOM, so a
  // file field in the Q&A set only produces a confusing "needs your answer".
